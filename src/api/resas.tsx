...
export const getPopulationData = async (prefCode: number) => {
  try {
    const response = await axios.get(`${baseUrl}/population/composition/perYear?prefCode=${prefCode}`, {
      headers: { "X-API-KEY": apiKey },
    });
    return response.data;
  } catch (error) {
    console.error(`Error fetching population data for prefecture code ${prefCode}:`, error);
    throw error;
  }
};
